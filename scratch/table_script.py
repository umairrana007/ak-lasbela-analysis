...
lines_to_check = {
    'Line 1': ({'7', '1', '2'}, {'0', '5', '9'}, '059'),
    'Line 2': ({'7', '1', '0'}, {'2', '5', '9'}, '259'),
    'Line 3': ({'7', '1', '5'}, {'2', '0', '9'}, '209'),
    'Line 4': ({'7', '1', '9'}, {'2', '0', '5'}, '205')
}

columns = ['GM', 'LS1', 'AK', 'LS2', 'LS3']

def find_counterpart_in_future(start_idx, counterpart_set):
    # Check the next 7 records (roughly a week)
    results = []
...
for line_name, (target_digits, counterpart_set, counterpart_name) in lines_to_check.items():
    ...
    for r_idx, row in enumerate(records):
        for i in range(4):
            ...
            if target_digits.issubset(pair_digits):
                ...
                print(f"{line_name} | {match_str}")
                if future_matches:
                    print(f"  --> Counterpart ({counterpart_name}) double found on: " + ", ".join(future_matches[:3]))
                else:
                    print(f"  --> No counterpart double found soon.")
