...
all_10_pairs = [
    ...
    ('719', '205'),
    ('120', '759'),
    ('105', '927'), # Note: digits are 1,0,5 | 9,2,7
    ('109', '527'),
    ('207', '159'),
    ('907', '125'),
    ('507', '129')
...
def find_counterpart_in_future(start_idx, counterpart_set):
    results = []
    for i in range(start_idx + 1, min(start_idx + 15, len(records))):
        row = records[i]
        for c_idx, num in enumerate(row['nums']):
            if num[0] in counterpart_set and num[1] in counterpart_set:
                results.append(f"{row['date']} ({row['day']}) -> {columns[c_idx]} [{num}]")
    return results

print("=== ALL 10 MATCHES ===")
for target_str, counterpart_str in all_10_pairs:
    target_digits = set(list(target_str))
    # Correct the counterpart digits just in case user typo'd 927 -> actually 7,9,2 are all valid digits from 7,1,2,0,5,9. Yes, all are valid digits.
    # 71.20.59 digits: 7, 1, 2, 0, 5, 9. 
    # Let's just use the string they gave.
    ...
    for r_idx, row in enumerate(records):
        for i in range(4):
            ...
            if target_digits.issubset(pair_digits):
                found_any = True
                match_str = f"{row['date']} ({row['day']}) | Aamne-Samne | {columns[i]}[{row['nums'][i]}] & {columns[i+1]}[{row['nums'][i+1]}]"
                future_matches = find_counterpart_in_future(r_idx, counterpart_set)
                if future_matches:
                    print(f"{target_str} | {match_str} | {counterpart_str} | " + ", ".join(future_matches[:1]))
                else:
                    print(f"{target_str} | {match_str} | {counterpart_str} | Wait")
    ...
    for r in range(len(records) - 1):
        for c in range(5):
            ...
            if target_digits.issubset(pair_digits):
                found_any = True
                match_str = f"{records[r]['date']} ({records[r]['day']}) & {records[r+1]['date']} ({records[r+1]['day']}) | Upar-Neechay | {columns[c]} [{records[r]['nums'][c]} & {records[r+1]['nums'][c]}]"
                future_matches = find_counterpart_in_future(r + 1, counterpart_set)
                if future_matches:
                    print(f"{target_str} | {match_str} | {counterpart_str} | " + ", ".join(future_matches[:1]))
                else:
                    print(f"{target_str} | {match_str} | {counterpart_str} | Wait")
    ...
    if not found_any:
        print(f"{target_str} | Koi match abhi tak nahi aaya | - | {counterpart_str} | Wait")
